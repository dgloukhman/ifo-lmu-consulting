...
2.  **Execution**: The main analysis scripts are `univariate.R` and `multivariate.R`.
    - `univariate.R` tests the predictive power of single time series (one question from one industry) for the main KLD index.
    - `multivariate.R` tests the combined predictive power of all questions within a single industry for the main KLD index.
3.  **Plotting**: The `plotting.R` script is the main entry point that runs the univariate analysis for both "simple" and "instantaneous" forecast types and generates the corresponding plots. It saves the plots as PNG files in the root directory.
